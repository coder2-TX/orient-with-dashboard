@php
  ...
  $title = data_get($hero, 'title.en')
    ?: 'ORIENT YEMEN';

  $tagline = data_get($hero, 'third_line.en')
  ...
  $defaultSlides =
    HomeHero::publicDefaultSlideUrls('en');

  $dbSlides = is_array($hero?->slides_en)
    ? array_values(array_filter($hero->slides_en))
    : [];
...
>
  <div class="oy-hero__slides">
    @foreach($slides as $i => $url)
      <div
        class="oy-hero__slide {{ $i === 0 ? 'is-active' : '' }}"
  ...
  <div class="oy-hero__content">
    <div class="oy-hero__content-inner">
      ...
      <div
        class="oy-hero__controls oy-reveal oy-delay-4"
        aria-label="Slider Controls"
      >
        <button
          class="oy-hero__arrow oy-hero__arrow--prev"
          type="button"
          aria-label="Previous"
        >
        ...
        >
          @foreach($slides as $i => $url)
            <button
              class="oy-hero__dot {{ $i === 0 ? 'is-active' : '' }}"
              type="button"
        ...
        <button
          class="oy-hero__arrow oy-hero__arrow--next"
          type="button"
          aria-label="Next"
        >
