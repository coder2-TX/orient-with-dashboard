...
@php
  ...
  // switch by route-name: site.* <-> site_en.*
  if ($currentRoute) {
  ...
  if ($targetRoute && \Illuminate\Support\Facades\Route::has($targetRoute)) {
    ...
    // fallback by path (/en prefix)
    $path = request()->path(); // '' for '/'
...
<header class="oy-header">
  ...
  <div class="oy-header__drawer" aria-hidden="true">
    ...
    <aside class="oy-header__drawerPanel" role="dialog" aria-modal="true" aria-label="القائمة">
      ...
      <a class="oy-btn oy-btn--primary oy-header__drawerCta" href="{{ route('site.contact') }}">تواصل معنا</a>
    </aside>
  </div>
</header>
